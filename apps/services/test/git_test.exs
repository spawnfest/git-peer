defmodule GitPeer.Services.GitTest do
  ...
  alias Git, as: GitCli
  alias GitPeer.Services.Git, as: GitService
  ...
  test "Get current repo directory", %{path: path} do
    File.cd(path)
    {:ok, repo_path} = GitService.get_current_repo_directory()
  ...
  test "Get current repo", %{path: path} do
    File.cd(path)
    {:ok, %{path: repo_path}} = GitService.get_current_repo()
  ...
  test "Get diff path", %{gitex_repo: gitex_repo, git_cli_repo: git_cli_repo} do
    ...
    {:ok, actual_files} = GitService.get_diff_files(git_cli_repo)
  ...
  test "Get current branch", %{git_cli_repo: git_cli_repo} do
    branch = "master"
    {:ok, actual_branch} = GitService.get_current_branch(git_cli_repo)
  ...
  test "Get current hash", %{git_cli_repo: git_cli_repo, gitex_repo: gitex_repo} do
    ...
    {:ok, actual_hash} = GitService.get_current_hash(git_cli_repo)
